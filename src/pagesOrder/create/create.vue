...
const orderDetail = ref<OrderPreResult>()
...
const getMemberOrderPre = async () => {
  ...
  if (props.skuId && props.count) {
    const result = await getMemberOrderPreNowAPI({ skuId: props.skuId, count: props.count })
...
const selectAddress = computed(() => {
  return addressStore.selectedAddress || orderDetail.value?.userAddresses.find(v => v.isDefault)
})
...
const onSubmitPay = async () => {
  ...
  const result = await postMemberOrderAPI({
    ...
    goods: orderDetail.value?.goods.map((v) => ({ count: v.count, skuId: v.skuId })) !,
    payChannel: 2,
    payType: 1
  })
  uni.redirectTo({
...
<template>
  <scroll-view scroll-y class="viewport">
    ...
    <view class="settlement">
      <view class="item">
        ...
        <text class="number symbol">{{ orderDetail?.summary?.totalPayPrice.toFixed(2) }}</text>
      </view>
      <view class="item">
        <text class="text">运费: </text>
        <text class="number symbol">{{ orderDetail?.summary?.postFee.toFixed(2) }}</text>
      </view>
    </view>
  </scroll-view>
  ...
  <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
    <view class="total-pay symbol">
      <text class="number">{{ orderDetail?.summary?.totalPayPrice.toFixed(2) }}</text>
    </view>
    <view class="button" :class="{ disabled: !selectAddress.id }" @tap="onSubmitPay"> 提交订单 </view>
  </view>
</template>
