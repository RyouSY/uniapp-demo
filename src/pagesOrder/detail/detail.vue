...
import { getPayWxPayMiniPayAPI, getPayMockAPI, getMemberOrderConsignmentByIdAPI, putMemberOrderReceiptByIdAPI, getMemberOrderLogisticsByIdAPI, deleteMemberOrderAPI } from '@/services/pay';
...
// 猜你喜欢
const { guessRef, onScrolltolower } = useGetGuessApi()
// 弹出层组件
    ...
    <template v-if="true">
      ...
      <view class="overview" :style="{ paddingTop: safeAreaInsets!.top + 20 + 'px' }">
        ...
        <template v-if="order?.orderState === OrderState.DaiFuKuan">
          ...
          <view class="button" @tap="payMemt">去支付</view>
        </template>
        <!-- 其他订单状态:展示再次购买按钮 -->
        <template v-else>
          <!-- 订单状态文字 -->
          ...
          <view class="button-group">
            ...
            <view v-if="isDev && order?.value?.orderState === OrderState.DaiFaHuo" class="button" @tap="onOrderSend"> 模拟发货 </view>
      ...
      <view class="shipment">
        <!-- 订单物流信息 -->
        <view v-for="item in logList" :key="item.id" class="item">
          <view class="message">
            {{ item.text }}
          </view>
      ...
      <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
        <!-- 待付款状态:展示支付按钮 -->
        <template v-if="order?.orderState === OrderState.DaiFuKuan">
          <view class="button primary" @tap="payMemt"> 去支付 </view>
        ...
        <template v-else>
          ...
          <view class="button delete" v-if="order?.orderState >= OrderState.DaiPingJia" @tap="onDelete"> 删除订单 </view>
        </template>
      </view>
    </template>
    <template v-else>
